...
class User(db.Model):
    ...
    def __init__(
        self, firstname, lastname, username, email, password, institution_id=None
    ):
...
class Admin(User):
    ...
    def __init__(self, firstname, lastname, username, email, password):
        super().__init__(
            firstname, lastname, username, email, password, institution_id=None
        )
        self.role = "admin"
...
# -------------------- Institution --------------------
# Added: contact_person, contact_email, phone, status  (wireframe Image 2)
class Institution(db.Model):
    __tablename__ = "institution"
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False)
    code = db.Column(db.String(20), nullable=False, unique=True)
    contact_person = db.Column(db.String(100), nullable=True)  # "Contact Person" field
    contact_email = db.Column(db.String(120), nullable=True)  # "Contact Email" field
    phone = db.Column(db.String(30), nullable=True)  # "Phone" field
...
class Participant(db.Model):
    ...
    institution_id = db.Column(
        db.Integer, db.ForeignKey("institution.id"), nullable=False
    )
    institution = db.relationship(
        "Institution", backref=db.backref("participants", lazy=True)
    )
...
# -------------------- Season --------------------
# Added: status, reg_open, reg_close, start_date, end_date  (wireframe Image 3)
class Season(db.Model):
    ...
    status = db.Column(
        db.String(20), default="planning"
    )  # planning|active|closed|archived
    reg_open = db.Column(db.Date, nullable=True)  # Registration Opens
    reg_close = db.Column(db.Date, nullable=True)  # Registration Closes
    start_date = db.Column(db.Date, nullable=True)  # Season Start
    end_date = db.Column(db.Date, nullable=True)  # Season End
...
class Event(db.Model):
    ...
    name = db.Column(db.String(100), nullable=False)
    description = db.Column(db.String(200))
    event_type = db.Column(db.String(20))  # run|walk|mixed|other
...
# -------------------- Season-Event Bridge --------------------
# Added: status  (wireframe Image 1 — Active / Inactive per event in season)
class SeasonEvent(db.Model):
    ...
    status = db.Column(db.String(20), default="active")  # active | inactive
    start_date = db.Column(db.Date, nullable=True)  # event start within season
    end_date = db.Column(db.Date, nullable=True)  # event end within season
    ...
    __table_args__ = (
        db.UniqueConstraint("season_id", "event_id", name="_season_event_uc"),
    )
...
# -------------------- Stage --------------------
# Added: distance  (wireframe Image 1 — Stage table has Distance column)
class Stage(db.Model):
    __tablename__ = "stage"
    id = db.Column(db.Integer, primary_key=True)
    season_event_id = db.Column(
        db.Integer, db.ForeignKey("season_event.id"), nullable=False
    )
    stage_number = db.Column(db.Integer, nullable=True)
    distance = db.Column(db.String(20), nullable=True)  # e.g. "5K", "10K", "3K"
    location = db.Column(db.String(100), nullable=True)
    stage_date = db.Column(db.Date, nullable=True)

    season_event = db.relationship(
        "SeasonEvent", backref=db.backref("stages", lazy=True)
    )
    __table_args__ = (
        db.UniqueConstraint("season_event_id", "stage_number", name="_stage_uc"),
    )
...
class Registration(db.Model):
    ...
    participant = db.relationship(
        "Participant", backref=db.backref("registrations", lazy=True)
    )
    season_event = db.relationship(
        "SeasonEvent", backref=db.backref("registrations", lazy=True)
    )
    __table_args__ = (
        db.UniqueConstraint(
            "participant_id", "season_event_id", name="_part_seasonevent_uc"
        ),
    )
...
class BibNo(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("season_id", "bib_value", name="_bibNo_season_uc"),
    )
...
class BibTag(db.Model):
    ...
    season = db.relationship("Season")
    __table_args__ = (
        db.UniqueConstraint("season_id", "bib_value", name="_bibTag_season_uc"),
    )
...
class BibNoAssignment(db.Model):
    ...
    registration_id = db.Column(
        db.Integer, db.ForeignKey("registration.id"), primary_key=True
    )
    bib_no_id = db.Column(db.Integer, db.ForeignKey("bibNo.id"), primary_key=True)
    ...
    registration = db.relationship(
        "Registration", backref=db.backref("bib_no_assignments", lazy=True)
    )
...
class BibTagAssignment(db.Model):
    ...
    bib_tag_id = db.Column(db.Integer, db.ForeignKey("bibTag.id"), primary_key=True)
    assign_date = db.Column(db.DateTime, default=datetime.utcnow)
    status = db.Column(db.String(20), default="active")  # active|lost|replaced
    registration = db.relationship(
        "Registration", backref=db.backref("bib_tag_assignments", lazy=True)
    )
...
class Result(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("registration_id", "stage_id", name="_result_reg_stage_uc"),
    )
