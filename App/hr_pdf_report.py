...
# ── Chart helpers (same as admin report) ─────────────────────────────────────
...
# ── Main HR report generator ──────────────────────────────────────────────────
def generate_hr_report(institution, stats, participants, stage_completion,
                       division_counts, gender_split, age_groups):
    """
    Returns a PDF bytes object with institution‑specific report.
    """
    buf    = io.BytesIO()
    st     = _styles()
    margin = 1.5 * cm

    doc_title = f'{institution.name} – Performance Report'
    doc = BaseDocTemplate(
        buf, pagesize=A4,
    ...
    story.append(Paragraph(institution.name, st['cover_title']))
    story.append(Spacer(1, 0.3 * cm))
    story.append(Paragraph('Institution Performance Report', st['cover_sub']))
    story.append(Spacer(1, 0.5 * cm))

    # Stats band
    total_p = stats['total_participants']
    reg_c   = stats['reg_count']
    part_c  = stats['part_count']
    no_show = stats['no_show_count']
    part_rate = round(part_c / reg_c * 100, 1) if reg_c else 0

    cover_stats = Table([[
        Paragraph(str(total_p),  ParagraphStyle('cv', fontSize=28, textColor=NAVY, fontName='Helvetica-Bold', alignment=TA_CENTER)),
        Paragraph(str(reg_c),    ParagraphStyle('cv', fontSize=28, textColor=NAVY, fontName='Helvetica-Bold', alignment=TA_CENTER)),
        Paragraph(f'{part_rate}%', ParagraphStyle('cv', fontSize=28, textColor=NAVY, fontName='Helvetica-Bold', alignment=TA_CENTER)),
        Paragraph(str(no_show),  ParagraphStyle('cv', fontSize=28, textColor=NAVY, fontName='Helvetica-Bold', alignment=TA_CENTER)),
    ], [
        ...
        Paragraph('Participation',ParagraphStyle('cl', fontSize=9, textColor=MUTED, alignment=TA_CENTER)),
        Paragraph('No‑Shows',     ParagraphStyle('cl', fontSize=9, textColor=MUTED, alignment=TA_CENTER)),
    ...
    story.append(_metric_row([
        ...
        (reg_c,   'Registered This Season'),
        (part_c,  'Participated'),
        (no_show, 'No‑Shows'),
        (f'{part_rate}%', 'Participation Rate'),
    ], st))
    story.append(Spacer(1, 0.5 * cm))

    # Participant Roster table
    story.append(Paragraph('Participant Roster', st['subsection']))
    if participants:
        roster_data = [[
            Paragraph('Name',       st['tbl_hdr']),
            Paragraph('Email',      st['tbl_hdr']),
            Paragraph('Division',   st['tbl_hdr']),
            Paragraph('Status',     st['tbl_hdr']),
        ]]
        for p in participants:
            status = 'Participated' if p.has_result else 'No‑Show'
            roster_data.append([
                Paragraph(f"{p.first_name} {p.last_name}", st['tbl_cell_l']),
                Paragraph(p.email or '-',                 st['tbl_cell']),
                Paragraph(p.division or '-',              st['tbl_cell']),
                Paragraph(status,                         st['tbl_cell']),
            ])
        col_w = [(W - 3*cm) * x for x in [0.35, 0.25, 0.20, 0.20]]
        roster_tbl = Table(roster_data, colWidths=col_w, repeatRows=1)
        ...
        story.append(Paragraph('No participants found.', st['body']))
    ...
    # PAGE 3 — CHARTS
    # ══════════════════════════════════════════════════════════════════════════
    story.append(Paragraph('Performance Analytics', st['section']))
    story.append(HRFlowable(width='100%', thickness=1, color=BLUE, spaceAfter=12))

    # Participation status pie chart
    if part_c + no_show > 0:
        story.append(Paragraph('Participation Status', st['subsection']))
        pie_labels = ['Participated', 'No‑Show']
        pie_vals   = [part_c, no_show]
    ...
    if division_counts:
        story.append(Paragraph('Participants by Division', st['subsection']))
        div_labels = list(division_counts.keys())
        div_values = list(division_counts.values())
        fig = _bar_chart(div_labels, div_values,
                         'Participants per Division', colour='#1A56A0')
        story.append(_fig_to_image(fig, width_cm=14))
        story.append(Spacer(1, 0.4 * cm))

    # Stage completion for Urban Challenge (if results exist)
    if stage_completion:
        story.append(Paragraph('Urban Challenge Stage Completion', st['subsection']))
        s_labels = [f'Stage {s["stage"]}' for s in stage_completion]
        s_vals   = [s['percentage'] for s in stage_completion]
        fig = _horizontal_bar_chart(s_labels, s_vals,
                                    'Stage Completion Rate (%)', colour='#C8963E')
        story.append(_fig_to_image(fig, width_cm=14))
        story.append(Spacer(1, 0.4 * cm))
        ...
        for s in stage_completion:
            sc_data.append([
                ...
                Paragraph(f'{s["percentage"]}%', st['tbl_cell']),
            ])
    ...
    notes = [
        ...
        'No‑Show includes participants registered but with no result recorded.',
        ...
        'Data is sourced directly from the CariFin Data Engagement Dashboard database.',
    ]
