from App.models import Participant, Registration, Result, Institution, Stage, SeasonEvent, Season, Event
...
def get_hr_stats(institution_id):
    ...
    div_rows = db.session.query(
        ...
        Participant.division != ''
    ).group_by(Participant.division).order_by(func.count(Participant.id).desc()).all()
    ...
    gender_rows = db.session.query(
        ...
        Participant.sex != ''
    ).group_by(Participant.sex).all()

    g_total = sum(r[1] for r in gender_rows) or 1
    gender_data = [{'sex': r[0].upper(), 'count': r[1],
                    'pct': round(r[1] / g_total * 100, 1)} for r in gender_rows]
    ...
    if season_id:
        ...
        if urban:
            se = SeasonEvent.query.filter_by(season_id=season_id, event_id=urban.id).first()
            if se:
                stages = Stage.query.filter_by(season_event_id=se.id).order_by(Stage.stage_number).all()
                inst_reg_ids = [r.id for r in db.session.query(Registration.id)
                                .join(Participant, Registration.participant_id == Participant.id)
                                .filter(Registration.season_event_id == se.id,
                                        Participant.institution_id == institution_id).all()]
                total_reg = len(inst_reg_ids)
                for s in stages:
                    if total_reg == 0:
                        stage_completion.append({'stage': s.stage_number, 'completion': 0, 'completed': 0, 'total': 0})
                        continue
                    completed = db.session.query(func.count(func.distinct(Result.registration_id)))\
                        .filter(Result.stage_id == s.id,
                                Result.registration_id.in_(inst_reg_ids)).scalar() or 0
    ...
    participation_rate = round(len(participated_ids) / len(registered_ids) * 100, 1) if registered_ids else 0
...
def get_available_events(institution_id):
    ...
    for se in season_events:
        ...
        if event:
            events.append({'id': se.id, 'name': event.name, 'date': se.start_date or 'TBD'})
    return events
...
def register_participants(participant_ids, season_event_id):
    ...
    for pid in participant_ids:
        if not Registration.query.filter_by(participant_id=pid, season_event_id=season_event_id).first():
            db.session.add(Registration(participant_id=pid, season_event_id=season_event_id))
            count += 1
    db.session.commit()
    return count