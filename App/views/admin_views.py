    ...
    try:
        ...
        for _, row in df.iterrows():
            try:
                ...
                sex   = str(row[col_sex]).strip() if col_sex and pd.notna(row.get(col_sex)) else None
                div   = str(row[col_div]).strip() if col_div and pd.notna(row.get(col_div)) else None
                ...
                if any(np_.id is None for np_ in new_participants):
...
# ================== PDF REPORT ==================
@admin_views.route('/admin/report/season-pdf')
@jwt_required()
def season_pdf_report():
    """Generate and download a full PDF season report."""
    if current_user.role != 'admin':
        return "Access Denied", 403
    ...
    if not current_season:
        flash('No season found to generate report for.', 'danger')
        return redirect(url_for('admin_views.dashboard'))

    season_id = current_season.id

    from App.controllers.admin_controller import (
        get_institution_stats, get_stage_completion,
        get_participation_by_institution, get_participation_status_breakdown,
        get_stage_funnel, get_gender_split, get_age_group_distribution,
    )

    institution_stats     = get_institution_stats(season_id, filter_event, None, filter_inst)
    stage_completion      = get_stage_completion(season_id, filter_event, filter_inst) or []
    participation_by_inst = get_participation_by_institution(season_id, filter_event, None, filter_inst) or []
    status_breakdown      = get_participation_status_breakdown(season_id, filter_event, None, filter_inst) or {}
    stage_funnel          = get_stage_funnel(season_id, filter_event, filter_inst) or {}
    gender_split          = get_gender_split(season_id, filter_event, filter_inst) or []
    age_groups            = get_age_group_distribution(season_id, filter_event, filter_inst) or []

    try:
        from App.pdf_report import generate_season_report
        pdf_bytes = generate_season_report(
    ...
    filename = f'carifin_season_{current_season.year}_report.pdf'
    return send_file(
...
def export_roster():
    ...
    for p in participants:
        writer.writerow([
            p.first_name, p.last_name, p.email or '',
            p.institution.name if p.institution else '',
            p.division or '', p.sex or '',
...
def export_results():
    ...
    query = db.session.query(
        Participant.first_name, Participant.last_name, Institution.code,
        Event.name.label('event_name'), Stage.stage_number,
        Result.finish_time, Result.placement
        ...
        try:
            ...
            if not existing:
                db.session.add(Participant(
                    ...
                    email=email, institution_id=inst.id
                ))
                created += 1
            else:
                skipped += 1
